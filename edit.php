...
class embed_form extends moodleform
{
    public function definition()
    {
        ...
        $this->add_action_buttons(true, get_string('editembed', 'local_embedmanager'));
    }
}

$mform = new embed_form();
$mform->set_data($DB->get_record('embed', ['id' => $id]));
...
$PAGE->set_url(new moodle_url('/local/embedmanager/edit.php'));
$PAGE->set_context(context_system::instance());
$PAGE->set_title(get_string('editembed', 'local_embedmanager'));
$PAGE->set_heading(get_string('editembed', 'local_embedmanager'));
$PAGE->set_pagelayout('standard');

// 添加面包屑导航
$PAGE->navbar->add(get_string('embedmanager', 'local_embedmanager'), new moodle_url('/local/embedmanager/index.php'));
$PAGE->navbar->add(get_string('editembed', 'local_embedmanager'));
